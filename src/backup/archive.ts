...
export async function writeBackupBundle(params: {
  ...
  const rootManifest = {
    ...
    totals: clientResults.reduce((acc, r) => {
      // ⚡ Bolt Performance Optimization:
      // Single-pass aggregation replacing O(4N) multiple .filter().length calls
      // Reduces array traversals and eliminates intermediate allocations.
      acc[r.status]++;
      return acc;
