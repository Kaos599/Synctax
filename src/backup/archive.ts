...
export async function writeBackupBundle(params: {
  ...
  const rootManifest = {
    ...
    // ⚡ Bolt: Replaced multiple .filter().length calls (O(4N) and intermediate arrays)
    // with a single .reduce() pass (O(N) and no intermediate allocations) to efficiently compute totals.
    totals: clientResults.reduce(
      ...
      { clients: 0, success: 0, partial: 0, skipped: 0, failed: 0 }
    ),
  };
