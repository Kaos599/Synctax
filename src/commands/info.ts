...
export async function statusCommand() {
  ...
  // ⚡ Bolt Optimization: Parallelize adapter file reads across clients to drastically speed up sync status checks
  const results = await Promise.all(Object.entries(config.clients).map(async ([id, clientConf]) => {
    ...
    try {
      ...
      return { adapterName: adapter.name, success: true, inSync, driftDetails };
    } catch (e: any) {
      return { adapterName: adapter.name, success: false, error: e.message };
    }
  }));
...
export async function doctorCommand(options: any): Promise<boolean> {
  ...
  try {
    ...
    // ⚡ Bolt Optimization: Execute adapter detection concurrently across all configured clients for faster execution
    const results = await Promise.all(Object.entries(config.clients).map(async ([id, clientConf]) => {
      if (!clientConf.enabled) return null;
      const adapter = adapters[id];
      if (!adapter) {
        return { id, missingAdapter: true };
      }
      const detected = await adapter.detect();
      return { id, missingAdapter: false, adapterName: adapter.name, detected };
    }));
...
export async function infoCommand() {
  ...
  // ⚡ Bolt Optimization: Parallelize adapter detection and reading to significantly speed up info gathering
